...
export const downloadVisitingCard = createAsyncThunk(
  ...
  async ({ employeeId, style }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:3007/api/visiting-cards/download/${employeeId}/${style}`,
        {
          headers: {
      ...
      link.href = url;
      link.setAttribute("download", `${employeeId}_${style}_card.pdf`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);

      return { employeeId, style, message: "Card downloaded successfully" };
    } catch (error) {
