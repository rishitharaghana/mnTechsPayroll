...
const generateUniqueName = () =>
  `datePicker_${Math.random().toString(36).substr(2, 9)}`;
...
const DatePicker = ({
  ...
  return (
    <div className="relative w-full max-w-sm">
      ...
      {/* Input */}
      <div
        className="w-full flex items-center justify-between px-3 py-2 bg-white text-slate-700 border border-slate-300 rounded-lg shadow-sm focus-within:ring-2 focus-within:ring-teal-600 cursor-pointer"
            ...
            <div className="grid grid-cols-3 gap-2 mb-4 max-h-48 overflow-y-auto">
              {Array.from({ length: 100 }, (_, i) => currentYear - 50 + i).map(
                (year) => (
                  <button
                    key={year}
                    className="p-2 text-slate-700 text-sm hover:bg-teal-100 rounded"
            ...
            <>
              <div className="grid grid-cols-7 gap-1 mb-2">
                {days.map((day) => (
                  <div
                    key={day}
                    className="text-center font-semibold text-sm text-slate-700"
                  >
                    {day}
                  </div>
                ))}
