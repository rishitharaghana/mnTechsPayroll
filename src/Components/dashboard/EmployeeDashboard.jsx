...
const EmployeeDashboard = () => {
  ...
  // Quick actions
  const quickActions = [
    { label: 'Submit Leave Request', icon: FileText, color: 'bg-teal-600 hover:bg-teal-500', focusRing: 'focus:ring-teal-400', to: '/employee/leave-application' },
    { label: 'View Payslip', icon: DollarSign, color: 'bg-slate-700 hover:bg-slate-600', focusRing: 'focus:ring-slate-500', to: '/payslip' },
    { label: 'Track Attendance', icon: Clock, color: 'bg-teal-600 hover:bg-teal-500', focusRing: 'focus:ring-teal-400', to: '/attendance' },
    { label: 'View PF', icon: PiggyBank, color: 'bg-slate-700 hover:bg-slate-600', focusRing: 'focus:ring-slate-500', to: '/pf' },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8">
      {/* Welcome Section */}
      <div className="bg-white rounded-lg border border-slate-200 p-6 sm:p-8 mb-8 shadow-sm">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-800">
              Welcome back, {employee.name}!
            </h1>
            <p className="text-slate-500 text-base sm:text-lg">View your leave balance, attendance, and more.</p>
          </div>
          <div className="mt-4 md:mt-0 text-right">
            <p className="text-slate-500 text-sm sm:text-base">Today's Date</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        {leaveBalance.map((leave, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-white rounded-lg border border-slate-200 p-4 sm:p-6 shadow-sm hover:shadow-md transition-all duration-300 group relative"
              role="region"
            ...
            >
              ...
              <h3 className="text-slate-500 text-sm font-medium mb-1">{leave.type} Leave</h3>
              <p className="text-xl sm:text-2xl font-bold text-slate-900">{leave.remaining} days</p>
              <div className="absolute invisible group-hover:visible bg-slate-800 text-white text-xs rounded py-1 px-2 bottom-full mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg border border-slate-200 p-4 sm:p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <h2 className="text-lg sm:text-xl font-bold text-slate-900">Recent Leave Requests</h2>
            ...
            {recentLeaveRequests.map((request, index) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-slate-900 font-medium">{request.type} Leave ({request.id})</p>
                  <p className="text-slate-600 text-sm">
                    {request.from} to {request.to} ({request.days} days)
                  </p>
                  <p className="text-slate-500 text-xs">Status: {request.status} | {request.details}</p>
                </div>
                <NavLink
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-4 sm:p-6 shadow-sm">
          <h2 className="text-lg sm:text-xl font-bold text-slate-900 mb-6">Quick Actions</h2>
      ...
      {/* Attendance Status */}
      <div className="bg-white rounded-lg border border-slate-200 p-4 sm:p-6 mt-8 shadow-sm">
        <h2 className="text-lg sm:text-xl font-bold text-slate-900 mb-6">Attendance Status</h2>
        <div className="mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
            <div>
              <p className="text-slate-500 text-sm">Today's Status</p>
              <p className="text-xl sm:text-2xl font-bold text-slate-900">{attendanceStatus.today}</p>
            </div>
            <div className="text-right mt-4 sm:mt-0">
              <p className="text-slate-500 text-sm">Last Updated</p>
              <p className="text-slate-900 font-medium">{attendanceStatus.lastUpdated}</p>
            </div>
          </div>
        </div>
        <div className="space-y-4 max-h-96 overflow-y-auto">
          {recentAttendance.map((record, index) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-600 text-sm">Status: {record.status}</p>
