...
import { FileText, Download, Check, X, ChevronDown, ChevronUp, RefreshCw, User, Calendar, MapPin, DollarSign, AlertCircle } from "lucide-react";
...
const TravelExpenseApproval = () => {
  ...
  const fetchData = useCallback(() => {
    console.log("Fetching data:", { role: userRole, employee_id: profile?.employee_id, viewMode, page, statusFilter });
    if (!profile) {
  ...
  const handleStatusUpdate = (submissionId, status) => {
    ...
    dispatch(
      ...
      setShouldRefresh(true); // Refresh data after status update
    });
  ...
  console.log("Rendering data:", { viewMode, dataToDisplay, filteredData, statusFilter, submissions, history });
            ...
            <div className="flex gap-2 bg-gray-50 p-1 rounded-full shadow-sm">
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-full transition-all duration-300 ${
                  viewMode === "pending" ? "bg-gradient-to-r from-teal-600 to-teal-800 text-white shadow-md" : "bg-gray-50 text-gray-600 hover:bg-gray-100"
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-full transition-all duration-300 ${
                  viewMode === "history" ? "bg-gradient-to-r from-teal-600 to-teal-800 text-white shadow-md" : "bg-gray-50 text-gray-600 hover:bg-gray-100"
                } focus:outline-none focus:ring-2 focus:ring-teal-400`}
              >
                History
              </button>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        ...
        {!loading && filteredData.length === 0 && (
          <div className="text-center text-gray-500 text-sm font-medium py-8">
            No {["employee", "dept_head"].includes(userRole) ? "travel expense submissions" : viewMode === "pending" ? "pending submissions" : `history records${statusFilter ? ` for status: ${statusFilter}` : ""}`} found.
            {statusFilter === "Rejected" && viewMode === "history" ? " Try rejecting a pending submission to see it here." : ""}
          </div>
        )}
        {!loading && filteredData.length > 0 && (
                  ...
                  return (
                    <React.Fragment key={submission.id}>
                      <tr className="hover:bg-teal-50 transition-all duration-300 border border-slate-200">
                        ...
                        <td className="border border-slate-200 p-3">
                          <div className="text-sm text-slate-600">{formatDate(submission.travel_date)}</div>
                        </td>
                        <td className="border border-slate-200 p-3">
                          <div className="text-sm text-slate-600 truncate max-w-[120px]">{submission.destination || "N/A"}</div>
                        </td>
                        <td className="border border-slate-200 p-3 text-right">
                          <div className="font-semibold text-teal-800 text-sm">{formatAmount(submission.total_amount)}</div>
                        </td>
                        ...
                        <td className="border border-slate-200 p-3 text-center">
                          <button
                            ...
                            aria-label={isExpanded ? `Hide Details for Submission ${submission.id}` : `Show Details for Submission ${submission.id}`}
                          >
                              ...
                              <div className="p-4 border-t border-slate-200">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                  ...
                                  <div className="flex flex-col min-h-[50px] justify-between">
                                    <span className="text-xs font-medium text-slate-700 mb-1">Department</span>
                                    <span className="text-xs text-slate-600 leading-tight">{submission.department_name || "N/A"}</span>
                                  </div>
                                  {/* Purpose */}
                                  <div className="flex flex-col min-h-[50px] justify-between md:col-span-1">
                                    <span className="text-xs font-medium text-slate-700 mb-1">Purpose</span>
                                    <span 
                                  ...
                                  <div className="flex flex-col min-h-[50px] justify-between md:col-span-1 lg:col-span-1">
                                    <span className="text-xs font-medium text-slate-700 mb-1">Receipt</span>
                                  ...
                                  <div className="md:col-span-2 lg:col-span-3 space-y-1">
                                    <span className="text-xs font-medium text-slate-700 block mb-1">Expenses</span>
                                        ...
                                        {submission.expenses && submission.expenses.length > 0 ? (
                                          submission.expenses.slice(0, 3).map((exp, idx) => (
                                            <li key={idx} className="break-words">
                                              <span className="font-medium">{exp.purpose || "N/A"}:</span> {formatAmount(exp.amount)} on {formatDate(exp.expense_date)}
                                            </li>
                                          ))
                                        ) : (
                                  ...
                                  {/* Comment */}
                                  <div className="md:col-span-2 lg:col-span-3 space-y-1">
                                    <span className="text-xs font-medium text-slate-700 block mb-1">Comment</span>
                                  ...
                                  {/* Action Buttons */}
                                  {["hr", "super_admin"].includes(userRole) && viewMode === "pending" && submission.status === "Pending" && (
                                    <div className="md:col-span-2 lg:col-span-3 flex gap-2 justify-center pt-1">
                                      ...
                                      >
                                        <Check size={10} className="inline mr-1" /> Approve
                                      </button>
                                      <button
                                      ...
                                      >
                                        <X size={10} className="inline mr-1" /> Reject
                                      </button>
                                    </div>
                                  )}
                                </div>
                ...
                {Array.from({ length: paginationData?.totalPages || 1 }, (_, i) => i + 1).map((p) => (
                  <button
                    ...
                    className={`px-3 py-1 text-sm font-medium rounded-full transition-all duration-300 ${
                      page === p ? "bg-gradient-to-r from-teal-600 to-teal-800 text-white shadow-sm" : "bg-gray-50 text-gray-600 hover:bg-gray-100"
                    } focus:outline-none focus:ring-2 focus:ring-teal-400`}
                  >
                    {p}
                  </button>
