...
const EmployeeAvgHours = () => {
  ...
  useEffect(() => {
    if (dateRange.startDate > dateRange.endDate) {
      ...
      setDateRange({
        startDate: subDays(dateRange.endDate, 30),
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return (totalEmployeesHours || []).filter(
  ...
  const renderPieChart = () => {
      ...
      return (
        <div className="text-center">
          <p className="text-slate-500">
            No employee data available for pie chart.
          </p>
          <button
            onClick={retryFetch}
    ...
    const validEmployees = totalEmployeesHours.filter(
      (emp) =>
        emp.department_name &&
        emp.total_working_hours != null &&
        !isNaN(emp.total_working_hours) &&
        emp.total_working_hours > 0
    );
    if (!validEmployees.length) {
      ...
      return (
        <div className="text-center">
          <p className="text-slate-500">
            No valid working hours data available.
          </p>
          <button
            onClick={retryFetch}
            className="mt-2 px-4 py-2 bg-teal-600 text-white rounded hover:bg-teal-500"
    ...
    const deptHours = validEmployees.reduce((acc, emp) => {
      acc[emp.department_name] =
        (acc[emp.department_name] || 0) + emp.total_working_hours;
      return acc;
    }, {});
    console.log("deptHours:", deptHours);
    const totalHours =
      Object.values(deptHours).reduce((sum, h) => sum + h, 0) || 1;
    const colors = [
      "#14b8a6",
      "#3b82f6",
      "#ef4444",
    ...
    const slices = Object.entries(deptHours).map(([dept, hours], index) => {
      ...
      return (
        <g key={`slice-${dept}-${index}`}>
          <path
            ...
            onMouseOver={(e) =>
              setTooltip({
                x: e.clientX,
                y: e.clientY,
                text: `${dept}: ${((hours / totalHours) * 100).toFixed(
                  1
                )}% (${hours.toFixed(1)}h)`,
              })
            }
          ...
          >
            <title>{`${dept}: ${((hours / totalHours) * 100).toFixed(
              1
            )}%`}</title>
          </path>
          <text
          ...
          {Object.entries(deptHours).map(([dept, hours], index) => (
            ...
            >
              ...
              <span className="text-sm text-slate-700">
                {dept}: {((hours / totalHours) * 100).toFixed(1)}% (
                {hours.toFixed(1)}h)
              </span>
            </div>
          ))}
  ...
  const renderLineGraph = (data, title, color = "#14b8a6") => {
    if (
      ...
      data.trend_data.every((d) => d.hours == null)
    ) {
    ...
    const validTrendData = data.trend_data.filter(
      (d) => d.hours != null && !isNaN(d.hours)
    );
    ...
    const points = validTrendData
      .map((d, index) => {
        const x =
          margin.left + (index / (validTrendData.length - 1)) * graphWidth;
            ...
            {validTrendData.map((d, index) => (
              <g key={`point-${d.date}-${index}`}>
                <circle
                  cx={
                    margin.left +
                    (index / (validTrendData.length - 1)) * graphWidth
                  }
                  cy={
                    margin.top +
                    graphHeight -
                    (d.hours / maxHours) * graphHeight
                  }
                  r="4"
                  fill={color}
                  onMouseOver={(e) =>
    ...
    return (
      <div className="w-78/100 p-6">
        ...
        <p className="text-slate-600">
          This page is restricted to Super Admin and HR roles.
        </p>
      </div>
    );
  }
      ...
      <div className="flex justify-end">
        ...
        <PageBreadcrumb
          items={[
            ...
            {
              label: "Working Hours Analytics",
              link: "/admin/working-hours-analytics",
            },
          ]}
        />
      </div>
        ...
        {currentTab === "overview" && (
          <div className="space-y-6">
            ...
            {totalAverageHours ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 bg-teal-50 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-teal-600">
                      {totalAverageHours.total_average_working_hours != null &&
                      !isNaN(totalAverageHours.total_average_working_hours)
                        ? totalAverageHours.total_average_working_hours.toFixed(
                            1
                          )
                        : "0.0"}{" "}
                      hours
              ...
              <div className="text-center">
                <p className="text-slate-500">
                  No organization-wide data available.
                </p>
                <button
                  onClick={retryFetch}
                  className="mt-2 px-4 py-2 bg-teal-600 text-white rounded hover:bg-teal-500"
                        ...
                        (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <button
                                onClick={() =>
                                  handleFetchAverageHours(employee_id)
                                }
          ...
          <div className="space-y-6">
            <div className="bg-white/90 rounded-lg border border-slate-200/50 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
              <h2 className="text-xl font-bold text-slate-900 mb-4">
                ...
                {averageHours.employee_name || "Unknown"}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 bg-teal-50 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-teal-600">
                    {averageHours.average_working_hours != null &&
                    !isNaN(averageHours.average_working_hours)
            ...
            <div className="bg-white/90 rounded-lg border border-slate-200/50 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
              ...
              {renderLineGraph(
                averageHours,
                `Working Hours Trend for ${
                  averageHours.employee_name || "Employee"
                }`,
                "#3b82f6"
              )}
            </div>
